(function () {
  ...
  function ctaScrollMorph(pScroll) {
    return clamp01(remap(pScroll, 0.2, 0.8));
  }
  ...
  function applyFooterRibbonElements(pScroll, pDonation, atFooterZone) {
    ...
    if (layers.donate) {
      if (pDonation > 0.08) {
        ...
        layers.donate.classList.toggle("is-landing-cta", t < 0.42);
        layers.donate.classList.remove("cta-layout-landing", "cta-layout-ribbon");
        layers.donate.style.setProperty("--cta-morph", String(t));
        const ctaVisible = fadeOut(pDonation, 0, 0.1);
